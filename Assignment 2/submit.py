...
class DecisionTree:
    # Decision tree, a general overall wrapper to call the function `fit`
...
class Node:
    ...
    def process_leaf(self, my_words_idx, verbose):
        if verbose:
            print(all_words[my_words_idx[0]], end='')
        return my_words_idx[0]

    # Find out the intersection between the query and the word
    # Return the letter when match seen and return "_" when the match is not present
    # Basically, it returns "_ _ _ _ _ _" or any alphabet that fits
    ...
    # Calculating the entropy of the decision made at the node, not the leaf
    ...
    # Calculate the intersection, similar to reveal function but with different inputs
    ...
    # Similar to try_attr, but with different inputs
    def get_dict(self, my_words_idx, query):
        split_dict = {}
        count_dict = {}
    ...
    def process_node(self, my_words_idx, verbose):
        ...
        global all_words, size_limit
        best_split_dict = {}
        if self.depth == 0:
            query_idx = -1
            query = ""
            (best_split_dict, count_dict) = self.get_dict(my_words_idx, query)
        else:
            query_idx = 0
            size = len(my_words_idx)
            words_idx = my_words_idx
            if size >= size_limit:
                size = size_limit
                words_idx = np.random.choice(my_words_idx, size)
            (best_split_dict, count_dict) = self.get_dict(my_words_idx, all_words[words_idx[0]])
            min_entropy = self.get_entropy(np.array(list(count_dict.values())))
            query_idx = words_idx[0]
            for i in words_idx:
                (split_dict, count_dict) = self.get_dict(my_words_idx, all_words[i])
                entropy = self.get_entropy(np.array(list(count_dict.values())))
    ...
    # Fitting function called recursively
    def fit(self, my_words_idx, min_leaf_size, max_depth, fmt_str="    ", verbose=False):
        ...
        if len(my_words_idx)<=min_leaf_size or self.depth>=max_depth:
            self.is_leaf = True
            self.query_idx = self.process_leaf(self.my_words_idx, verbose)
            if verbose:
                print(' ')
        else:
            self.is_leaf = False
